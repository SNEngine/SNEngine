...
{
    public class DialogOnScreenNodeWorker : SNILWorker
    {
        public override void ApplyParameters(BaseNode node, Dictionary<string, string> parameters)
        {
            // Проверяем, что это действительно DialogOnScreenNode или его наследник
            if (!(node is DialogOnScreenNode dialogNode))
            {
                return;
            }
