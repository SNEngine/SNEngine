...
{
    public class PlaySoundNodeWorker : SNILWorker
    {
        public override void ApplyParameters(BaseNode node, Dictionary<string, string> parameters)
        {
            // Проверяем, что это действительно PlaySoundNode или его наследник
            if (!(node is PlaySoundNode playSoundNode))
            {
                return;
            }
            ...
            foreach (var kvp in parameters)
            {
                ...
                if (field != null)
                {
                    if (field.FieldType == typeof(AudioClip))
                    {
                        ...
                        string soundPath = SNILResourceFinder.FindResourcePath(kvp.Value, typeof(AudioClip));
                        if (!string.IsNullOrEmpty(soundPath))
                        {
                            AudioClip sound = AssetDatabase.LoadAssetAtPath<AudioClip>(soundPath);
                            if (sound != null)
                            {
                                field.SetValue(node, sound);
                            }
                            else
                            {
                                SNILDebug.LogWarning($"Could not load sound from path: {soundPath}");
                            }
                        }
                        else
                        {
                            SNILDebug.LogWarning($"Could not find sound with name or path: {kvp.Value}");
                        }
                    }
                    else
                    {
                ...
                {
                    // Если поле не найдено по имени, пробуем найти поле _sound (для PlaySoundNode)
                    if (kvp.Key.Equals("sound", System.StringComparison.OrdinalIgnoreCase) ||
                        kvp.Key.Equals("_sound", System.StringComparison.OrdinalIgnoreCase))
                    {
                        var soundField = fields.FirstOrDefault(f => 
                            f.Name.Equals("_sound", System.StringComparison.OrdinalIgnoreCase) ||
                            f.Name.Equals("AudioClip", System.StringComparison.OrdinalIgnoreCase));
                        
                        if (soundField != null && soundField.FieldType == typeof(AudioClip))
                        {
                            string soundPath = SNILResourceFinder.FindResourcePath(kvp.Value, typeof(AudioClip));
                            if (!string.IsNullOrEmpty(soundPath))
                            {
                                ...
                                if (sound != null)
                                {
                                    soundField.SetValue(node, sound);
                                }
                                else
                                {
                                    SNILDebug.LogWarning($"Could not load sound from path: {soundPath}");
